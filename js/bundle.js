...
function cards() {
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('https://6304aa4694b8c58fd7225d37.mockapi.io/diches').then(data => {
    data.forEach(_ref => {
      let {
        img,
        altimg,
...
function slides(_ref) {
  ...
  const slides = document.querySelectorAll(slide),
        ...
        sliderWepper = document.querySelector(wrapper),
        slidesField = document.querySelector(field),
        width = window.getComputedStyle(sliderWepper).width;
  ...
  sliderWepper.style.overflow = 'hidden';
...
`;
  ...
  const cutWidth = str => +str.replace(/\D/g, '');

  next.addEventListener('click', () => {
    if (offset == cutWidth(width) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += cutWidth(width);
    }
    ...
    correctedOpacity(dots);
  });
  prev.addEventListener('click', () => {
    if (offset == 0) {
      offset = cutWidth(width) * (slides.length - 1);
    } else {
      offset -= cutWidth(width);
    }
  ...
  dots.forEach(doc => {
    doc.addEventListener('click', e => {
      const sladeTo = e.target.getAttribute('data-slide-to');
      slideIndex = sladeTo;
      offset = cutWidth(width) * (sladeTo - 1);
      correctedOpacity(dots);
    });
  });

  function correctedOpacity(d) {
